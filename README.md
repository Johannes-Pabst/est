...
- untested! since there's no other ... to EST converter, it's hard to check the correctness. Working on it though!
- DST timezones not supported yet. Working on it though!
...
- "the non-prime-numbered minutes of the first full non-reversed hour after a solstice or equinox happen twice"
    ...
    - the minute from x:00:00 to x:00:59 is minute number 1.
    - clocks jump from x:y:59 to x:y:00 for y+1 not prime once per y in such hours and run at normal speed at all times
...
- "Countries may entre DST, but no time may pass there."
